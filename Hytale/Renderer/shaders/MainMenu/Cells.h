...
static constexpr const char* CELLS_FRAG = R"(#version 330 core
...
float disk(vec2 r, vec2 center, float radius) {
	return 1.0 - smoothstep( radius-0.001, radius+0.008, length(r-center));
}

void main()
{
	float t = u_Time*2.;
	vec2 r = (2.0*gl_FragCoord.xy - u_Size.xy) / u_Size.y;
	...
	pix -= smoothstep(0.1, 5.5, length(r));	
